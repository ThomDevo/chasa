...
@NamedQueries(value = {
        ...
        @NamedQuery(name="UserEvent.SelectListUserEventById", query="SELECT ue from UserEventsEntity ue where ue.eventsByIdEvent.idEvent = :idUserEvent AND ue.eventsByIdEvent.dateTimeEvent > CURRENT_TIMESTAMP AND ue.userEventStatus = true AND (lower(ue.usersByIdUser.lastName) LIKE CONCAT('%', :researchUser,'%') OR lower(ue.usersByIdUser.firstName) LIKE CONCAT('%', :researchUser,'%') OR lower(ue.usersByIdUser.lifrasNumber) LIKE CONCAT('%', :researchUser,'%') OR lower(ue.usersByIdUser.userPhone) LIKE CONCAT('%', :researchUser,'%') OR lower(ue.usersByIdUser.email) LIKE CONCAT('%', :researchUser,'%')) ORDER BY ue.usersByIdUser.lastName ASC")
})
